...
# Load the models
rf_cstocks = joblib.load(f'../models/rf_{task}.joblib')
nn_cstocks = load_model(f"../models/nn_{task}.keras")
...
# rf_explainer = shap.TreeExplainer(rf_cstocks, X_test_shap)
# rf_shap_values = rf_explainer(X_test_shap)

# # Save the explainer
# joblib.dump(rf_explainer, '../shap/cstocks/rf_explainer.joblib')
# # Save the SHAP values
# joblib.dump(rf_shap_values, '../shap/cstocks/rf_shap_values.joblib')

rf_shap_values = joblib.load('../shap/cstocks/rf_shap_values.joblib')
...
# nn_explainer = shap.KernelExplainer(model=lambda x: np.concatenate(nn_cstocks.predict(x), axis=1), data=X_test_scaled_shap)
...
plt.savefig('../results/figures/shap/summary_plot_VegC_NN.jpg', dpi=300, bbox_inches='tight')
plt.close()
shap.summary_plot(nn_shap_values[1][:], X_test_scaled_shap)
plt.savefig('../results/figures/shap/summary_plot_SoilC_NN.jpg', dpi=300, bbox_inches='tight')
plt.close()

shap.summary_plot(nn_shap_values[2][:], X_test_scaled_shap)
plt.savefig('../results/figures/shap/summary_plot_LitterC_NN.jpg', dpi=300, bbox_inches='tight')
plt.close()
