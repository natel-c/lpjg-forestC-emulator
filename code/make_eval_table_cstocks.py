...
for metric in ['nrmse', 'rel_bias', 'r2']:
    for scenario in df_nn['scenario'].unique():
        row = {
            '': metric.upper(),  
            'Scenario': scenario, 
            'VegC_NN': np.mean(df_nn.loc[(df_nn['target'] == 'VegC') & (df_nn['scenario'] == scenario), metric].values),
            'VegC_RF': np.mean(df_rf.loc[(df_rf['target'] == 'VegC') & (df_rf['scenario'] == scenario), metric].values),
            'SoilC_NN': np.mean(df_nn.loc[(df_nn['target'] == 'SoilC') & (df_nn['scenario'] == scenario), metric].values),
            'SoilC_RF': np.mean(df_rf.loc[(df_rf['target'] == 'SoilC') & (df_rf['scenario'] == scenario), metric].values),
            'LitterC_NN': np.mean(df_nn.loc[(df_nn['target'] == 'LitterC') & (df_nn['scenario'] == scenario), metric].values),
            'LitterC_RF': np.mean(df_rf.loc[(df_rf['target'] == 'LitterC') & (df_rf['scenario'] == scenario), metric].values),
           }
        result.append(row)
